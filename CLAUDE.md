...
**輸出格式「範本版」（2026-08-20 新增）**：「輸出訊號」模組的格式切換從「標籤版／精簡版」二選一，擴充成三個分頁多一個「範本版」——把精簡版內容丟給 AI，判斷其中「情境特定」而非「通用結構」的內容（平台名稱、產業別、目標客群、數字限制等），改寫成 `{{變數標籤||原始內容}}` 格式，變成可套用到不同情境的範本 Prompt；變數數量上限寫死 `TEMPLATE_MAX_VARS = 10`，由 prompt 指示 AI 自行篩選最重要的部分，不做程式端的二次裁切（裁切變數可能切壞 `{{}}` 結構或語意，交給 AI 一次判斷完整輸出比較安全）。**這個轉換需要語意判斷，不是規則能做的**，因此重用既有「AI 產生六個模組」共用的同一組服務商／模型／金鑰設定（`$('aiProvider')`／`$('aiModel')`／`$('aiKey')`）與 `callLLM()`（含 429/500/503/529 自動重試），沒有另外開一組 API 設定。

**狀態管理**：`templateText`／`templateSourceText`／`templateError`／`templateGenerating` 四個模組級變數；`templateSourceText` 記錄「產生當下」的精簡版原文，之後若欄位被編輯導致精簡版原文跟 `templateSourceText` 不一致，`renderOutput()` 會顯示「⚠ 已變更，這是舊版範本」提示但**不會自動重新呼叫 AI**（避免每個按鍵都打 API），需按「🔄 重新產生範本版」（只在範本版分頁顯示）手動觸發；切到範本版分頁時若偵測到內容已變更或從未產生過，才會自動觸發一次 `generateTemplate(false)`（`force=false`＝命中快取就不重打 API，`force=true`＝強制重新產生，給重新產生鈕用，因為 AI 每次判斷的變數可能不同、使用者可能想換一版）。
