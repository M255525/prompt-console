...
**配色主題切換（2026-08-20 新增）**：預設暗色琥珀之外，新增暗色青綠（`dark-teal`）、暗色紫夜（`dark-violet`）、淺色（`light`）三組主題，topbar 品牌旁的四個 `.theme-swatch` 圓點可切換，選擇存 `localStorage`（key `crispeTheme`）。**技術做法**：所有色彩全部走 `:root` CSS 自訂屬性（`--bg`／`--bg-alt`／`--panel`／`--panel-raised`／`--line`／`--line-soft`／`--text`／`--text-dim`／`--text-faint`／`--amber`／`--amber-soft`／`--amber-glow`／`--teal`／`--red`），每個主題是一個 `:root[data-theme="..."]{...}` 覆寫區塊（比 `:root` 多一層屬性選擇器、優先權自然較高，不用 `!important`）；預設主題（`dark-amber`）不掛 `data-theme` 屬性（省一次 DOM 操作，也讓「沒設定過」與「明確選了預設值」是同一種狀態）。**踩坑：原本有多處顏色直接寫死 `rgba(232,163,61,...)`／`rgba(79,184,166,...)`／`rgba(209,84,63,...)`（body 背景的柔光漸層、`.module.output`／`.module.ai-module`／`.led-node.ai-led.ready` 的發光陰影、`.warn-box` 底色、`.btn.primary:hover` 的亮色），這些切主題後仍然是舊主題的顏色，因為它們从沒有引用 CSS 變數——已全部改用 `color-mix(in srgb, var(--amber) 15%, transparent)` 這類寫法動態算出半透明/加亮版本，才能真正跟著主題走**（`color-mix()` 現代瀏覽器皆支援，2026 年可放心用）。避免用即時 JS 計算多個 rgba() 常數的笨方法。切換時額外同步更新 `<meta name="theme-color">`（各主題對應各自 `--bg` 色，影響手機瀏覽器網址列/PWA 啟動畫面色調）。**避免換頁閃色**：`<head>` 最前面（`<meta charset>` 之後）內嵌一小段同步 script，搶在 CSS 套用前就從 `localStorage` 讀出已存主題、直接設定 `<html data-theme="...">`，不等主程式 IIFE 跑完才切換。已用 Playwright 驗證四個主題的 CSS 變數與 `color-mix()` 陰影正確隨主題改變、`localStorage` 持久化重整後仍套用正確主題、320px 寬度下主題圓點與其他 nav 按鈕正常換行無溢出。**测試方法論踩坑**：用 Playwright 同一個 `evaluate` 呼叫裡連續點擊 4 個主題再讀 `getComputedStyle` 顏色，因為 `body{transition:background-color .2s}` 動畫尚未跑完就同步讀值，讀到的是過渡中間值而非最終色（4 次讀值全部一樣、疑似卡住）——這只是測試方法的假象，不是真的 bug；正確驗證法是改讀 CSS 自訂屬性本身（`getComputedStyle(document.documentElement).getPropertyValue('--bg')`，自訂屬性不參與 transition 動畫）或每次點擊後等一個 `requestAnimationFrame`。

**輸出格式「範本版」（2026-08-20 新增）**：「輸出訊號」模組的格式切換從「標籤版／精簡版」二選一，擴充成三個分頁多一個「範本版」——把精簡版內容丟給 AI，判斷其中「情境特定」而非「通用結構」的內容（平台名稱、產業別、目標客群、數字限制等），改寫成 `{{變數標籤||原始內容}}` 格式，變成可套用到不同情境的範本 Prompt；變數數量硬上限 `TEMPLATE_MAX_VARS_CAP = 10`（**同日追加：使用者可調整實際要求 AI 產生幾個，1~10 之間**，`#templateMaxVars` number input，超出範圍會自動 clamp 回 1~10，選擇存 `localStorage`（key `crispeTemplateMaxVars`），由 prompt 動態帶入指示 AI 自行篩選最重要的部分，不做程式端的二次裁切（裁切變數可能切壞 `{{}}` 結構或語意，交給 AI 一次判斷完整輸出比較安全）。**這個轉換需要語意判斷，不是規則能做的**，因此重用既有「AI 產生六個模組」共用的同一組服務商／模型／金鑰設定（`$('aiProvider')`／`$('aiModel')`／`$('aiKey')`）與 `callLLM()`（含 429/500/503/529 自動重試），沒有另外開一組 API 設定。

**狀態管理**：`templateText`／`templateSourceText`／`templateSourceMaxVars`／`templateError`／`templateGenerating` 五個模組級變數；`templateSourceText`／`templateSourceMaxVars` 分別記錄「產生當下」的精簡版原文與變數上限，之後若欄位被編輯或變數上限被調整，導致目前狀態跟這兩個快取值不一致，`renderOutput()` 會顯示對應的「⚠ 已變更」提示（兩種情境文字不同：欄位變更 vs 變數上限變更，優先判斷欄位變更）但**不會自動重新呼叫 AI**（避免每個按鍵/每次調整數字都打 API），需按「🔄 重新產生範本版」（只在範本版分頁顯示，跟「變數上限」輸入框一起用同一組 `.show` class 切換顯示邏輯）手動觸發；切到範本版分頁時若偵測到內容已變更或從未產生過，才會自動觸發一次 `generateTemplate(false)`（`force=false`＝命中快取（原文與變數上限都相符）就不重打 API，`force=true`＝強制重新產生，給重新產生鈕用，因為 AI 每次判斷的變數可能不同、使用者可能想換一版）。**變數上限輸入框的 `input` 事件只負責即時觸發 `renderOutput()`（讓「已變更」提示即時反映使用者正在打字的過程），真正的 clamp／持久化留給 `change`／`blur` 事件（`syncTemplateMaxVarsInput()`）——如果 clamp 邏輯掛在 `input` 事件上，使用者打字打到一半（例如刪掉重打）會被中途強制修正，體驗很差。**

**避免污染「已儲存的提示詞」**：儲存清單原本只認 `format` 為 `labeled`／`merged`（`buildTextFromFields()` 只有這兩種分支），如果使用者在範本版分頁按「儲存」，會把 `format` 存成 `'merged'` 而非 `'template'`（範本版是 AI 產出的衍生內容，不是欄位原文的固定格式，儲存清單只存六個模組的原始欄位值，不會存 AI 產生的範本文字本身——如果真存了 `format:'template'`，之後「載入」會發生 `buildTextFromFields` 沒有對應分支的問題）。載入已儲存項目時會清空 `templateText`／`templateSourceText`／`templateError`（新載入的欄位跟舊快取的範本版完全無關，不清空會出現快取誤配對）。

已用 Playwright mock `window.fetch` 驗證完整流程（產生／複製／欄位變更後出現舊版提示／重新產生／API 401 錯誤訊息／欄位清空退回預設提示／儲存不會污染成 `template` 格式）；**測試踩坑**：mock 拆除後忘記還原就再次觸發產生，曾意外打到 Anthropic 正式 API（用假金鑰，被 401 擋下、無實際扣費，但之後測試一律先設一個回傳 rejected promise 的假 `fetch` 擋著，避免重蹈覆轍）。manual.html 已同步新增「範本版」說明卡片。
