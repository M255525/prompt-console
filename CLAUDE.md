...
**目標客群快選（2026-08-20 新增）**：`Insight`（背景洞悉）模組下方新增 `.audience-block`——10 個內建目標客群 chip（`25-35歲上班族`／`中小企業主／經營者`／`學生族群`／`銀髮族／熟齡客群`／`新手父母／育兒家庭`／`電商網購消費者`／`B2B採購決策者`／`自由工作者／SOHO族`／`企業HR／人資主管`／`內部團隊／同事`）＋1 個特殊選項「沒有特定客群」（虛線外框視覺區隔，`data-value="不限定特定客群"`，明確表態「刻意不鎖定」而非漏填）。內建 chip 沿用既有 `.chip[data-target]` 全域 click 綁定（跟 `Personality` 模組的快速語氣選項同一套機制，appends `、` 分隔），不需另外寫邏輯。**補充自訂客群**是新增邏輯：`#audienceCustomInput` + `#audienceAddBtn`，輸入後存 `localStorage`（key `crispeCustomAudiences`，陣列 `{id,name}`），立即插入欄位＋重繪 chip 列；自訂 chip 用 `<span role="button" tabindex="0">`（非 `<button>`，因為內部要再包一個獨立可點擊的刪除 `<button class="audience-del">`——button 不能巢狀 button，故外層改用可鍵盤操作的 span）；刪除鈕 `stopPropagation()` 避免同時觸發插入。新增時會擋重複（比對 11 個內建值＋既有自訂清單），並用既有 `showToast()` 提示。已用 Playwright 驗證：11 個內建 chip 正確渲染與插入、自訂新增/持久化（重整頁面仍在）/刪除三個動作、320px 寬度下 `.audience-add-row` 正常換行無溢出。
